...
słupkowym porównującym accuracy `contains_all` vs `is_exactly`.
...
| `evaluate_trial(...)` / `compare_trials(...)` | dwa accuracy (`contains_all` + `is_exactly`) + czułość + swoistość (micro i per-kategoria); `TrialResult.plot()` rysuje wykres accuracy `contains_all` vs `is_exactly` (osobna komórka) |
